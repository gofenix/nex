defmodule AuthDemo.Pages.Login do
  ...
  def render(assigns) do
    ~H"""
    <div data-testid="auth-login-page" class="max-w-md mx-auto mt-24 p-8">
      <h1 class="text-2xl font-bold mb-6">Login</h1>

      <%= if @flash[:error] do %>
        <div data-testid="auth-login-flash-error" class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4">
          {@flash[:error]}
        </div>
      <% end %>

      <div class="bg-white rounded-lg shadow p-6">
        <form action="/login" method="post" hx-post="/login" hx-target="body" hx-push-url="/" data-testid="auth-login-form">
          <div class="mb-4">
