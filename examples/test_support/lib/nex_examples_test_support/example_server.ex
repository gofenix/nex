defmodule NexExamplesTestSupport.ExampleServer do
  use GenServer
  ...
  def wait_until_ready(server, %Config{} = config, timeout \\ @startup_timeout) do
    deadline = System.monotonic_time(:millisecond) + timeout
    do_wait_until_ready(server, Config.ready_url(config), deadline, "server not ready")
  end
  ...
  def init(config) do
    ...
    log_path = Artifacts.log_path(config)
    ...
    port =
      Port.open({:spawn_executable, mix}, [
        ...
        :exit_status,
        {:cd, config.cwd},
        {:env, env_for_port(config.env)},
        {:args, ["nex.dev", "--port", Integer.to_string(config.port), "--host", "127.0.0.1"]}
      ])
    ...
    IO.binwrite(log_device, "Starting #{config.slug} on #{Config.base_url(config)}\n")
