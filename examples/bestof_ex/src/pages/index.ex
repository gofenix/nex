defmodule BestofEx.Pages.Index do
  ...
  def render(assigns) do
    ~H"""
    <!-- Hero Section -->
    <section data-testid="bestof-ex-page" class="py-12 text-center border-b border-gray-200 bg-white">
  ...
  defp fetch_hot_projects(period) do
    interval_sql = days_ago_sql(period)
    ...
    {:ok, rows} = NexBase.sql("""
      ...
      LEFT JOIN bestofex_project_stats ps
        ...
        AND ps.recorded_at = #{interval_sql}
  ...
  defp fetch_featured(count) do
    {:ok, rows} = NexBase.sql("""
      ...
      LEFT JOIN bestofex_project_stats ps
        ON ps.project_id = p.id
        AND ps.recorded_at = #{days_ago_sql("today")}
      ORDER BY RANDOM()
      LIMIT $1
    """, [count])
  ...
  # Batch-fetch tags for a list of projects (avoids N+1)
  ...
  defp fetch_monthly_rankings(count) do
    {:ok, rows} = NexBase.sql("""
      ...
      LEFT JOIN bestofex_project_stats ps
        ON ps.project_id = p.id
        AND ps.recorded_at = #{month_start_sql()}
      ORDER BY COALESCE(p.stars - ps.stars, 0) DESC, p.stars DESC
      LIMIT $1
    """, [count])
  ...
  defp days_ago_sql("week") do
    date_sql("-7 day")
  end

  defp days_ago_sql("month") do
    date_sql("-30 day")
  end

  defp days_ago_sql(_period) do
    date_sql("-1 day")
  end

  defp month_start_sql do
    case NexBase.adapter() do
      :sqlite -> "DATE('now', 'start of month')"
      _ -> "DATE_TRUNC('month', CURRENT_DATE)"
    end
  end

  defp date_sql(offset) do
    case NexBase.adapter() do
      :sqlite -> "DATE('now', '#{offset}')"
      _ -> "CURRENT_DATE + INTERVAL '#{offset}'"
    end
  end
end
