# NexBase Demo: Architect's Manifesto (v0.3.3)
...
- **Intent > Implementation**: Page Actions MUST describe *what* the user is doing (`def create_task`), not *how* the server handles it (`def handle_post`).
...
- **Example**: `NexBaseDemo.Pages.Index` for `src/pages/index.ex`.
...
- **API Handlers**: Return `%Nex.Response{}` via `Nex.json/2`, `Nex.html/2`, etc.
...
- **dotenvy**: For local development, use `Dotenvy.source!(".env")` in `Application.start/2`.
...
- **Example Form**:
  ...
  <form hx-post="/api/tasks">
    <input name="title" placeholder="Enter task..." />
    <button type="submit">Add</button>
  </form>
  """
  ```

## 12. NexBase Database Integration
- **Query Builder**: Use `NexBase.from("table") |> NexBase.eq(:col, val) |> NexBase.run()`
- **Pattern**: Always pipe through `NexBase.run()` to execute
- **Example - Select**:
  ```elixir
...
## 14. Locality & Component Promotion
...
## 15. Database Schema Initialization
- **Auto-create**: Tables are created on application start via SQL in `Application.start/2`
- **Pattern**:
  ```elixir
  create_sql = "CREATE TABLE IF NOT EXISTS tasks (...)"
  NexBase.query!(create_sql, [])
  ```
