...
### Added
- **SaaS starter**: Added `mix nex.new my_app --starter saas` to generate a NexBase-backed starter with SQLite by default, session auth, protected dashboard pages, automatic schema bootstrap, and starter project CRUD.

### Fixed
- **Generated SaaS starter**: Added the SQLite adapter dependency required for the default SQLite path, aligned generated page actions with the `%Nex.Req{}` contract, and removed generated template issues that caused warnings or startup failures.

### Changed
- **Starter messaging**: Clarified in the CLI help, README, and website docs that `basic` remains the default starter while `saas` is the explicit product-oriented option.

## [0.4.1] - 2026-03-14

### Fixed
- **Installer archive compilation**: Replaced compile-time `VERSION` file reads in `Mix.Tasks.Nex.New` with runtime version lookup from application metadata, fixing `mix archive.install hex nex_new` failures when `VERSION` is not present in installer build paths.

### Changed
- **Updated dependency**: `nex_core ~> 0.4.1`

## [0.3.9] - 2026-02-21

### Changed
- **Updated dependency**: `nex_core ~> 0.3.9`

## [0.3.8] - 2026-02-20

### Changed
- **Updated dependency**: `nex_core ~> 0.3.8`

## [0.3.7] - 2026-02-20

### Changed
- **Updated dependency**: `nex_core ~> 0.3.7`

## [0.3.6] - 2026-02-20

### Changed
- **Updated dependency**: `nex_core ~> 0.3.6`
...
### Changed
- **SSE Handler**: SSE endpoints are detected by checking for `stream/2` or `stream/1` function
