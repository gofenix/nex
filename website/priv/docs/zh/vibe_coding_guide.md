...
Nex 是专门为 **AI 时代**设计的框架。其简洁的设计和高度局部的结构，使得它成为了 AI 辅助编程（如使用 Claude, GPT-4, Cursor 等）的理想平台。
...
### 极简的 DSL
Nex 只有 `mount`、`render` 和普通的 Elixir 函数作为 Action。这种极少的概念集合让 AI 能够非常精准地遵循框架规范。
